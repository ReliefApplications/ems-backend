...
const errors = {
  ...
  invalidAddFormArguments:
    'Form should either correspond to a new resource or existing resource.',
  invalidEditResourceArguments:
    'Either fields or permissions must be provided.',
  ...
  invalidAddStepArguments:
    'Step type must be an available type and linked workflow ID must be provided.',
  invalidEditStepArguments:
    'Either name, type, content or permissions must be provided.',
  ...
  invalidPublishNotificationArguments:
    'Action, content and channel arguments must all be provided.',
  invalidCORS:
  ...
  missingRelatedField(name: string) {
    return `Please add a related name to ${name} question.`;
  },
  invalidUserUpload: 'Please provide email and role for each user.',
  missingFile: 'No file detected.',
  dataFieldDuplicated(name: string) {
    return `Data name duplicated : ${name}. Please provide different value names for all questions.`;
  },
  relatedNameDuplicated(name: string) {
    return `Related name duplicated : ${name}. The target Resource already contains a field with that name. Please provide a different name.`;
  },
  dataFieldCannotBeDeleted(name: string) {
    return `Data field cannot be deleted : ${name}. Some inherited forms implement this field.`;
  },
  coreFieldMissing(name: string) {
    return `Core field missing : ${name}. Please implement this field.`;
  },
  invalidConversion: 'Cannot convert this record to this target form type.',
  usageOfProtectedName:
    'This name is protected and cannot be used. Please choose a different name.',
  invalidAddApplicationName:
    'The name can only consist of alphanumeric characters. Please choose a different name.',
  invalidEmailsInput: 'Wrong format detected. Please provide valid emails.',
  invalidPaginationArguments:
    'Please provider valid integers for first and offset arguments.',
  invalidAddApiConfigurationArguments: 'API name must be provided.',
  invalidEditApiConfigurationArguments:
  ...
  fileSizeLimitReached: 'File size exceeds 5MB',
