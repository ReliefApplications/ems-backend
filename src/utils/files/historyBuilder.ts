...
/** Enum for text name of each header */
enum headersText {
  date = 'Date',
  displayName = 'Field',
...
const historyBuilder = async (
  ...
  const worksheet = workbook.addWorksheet(`Record history ${meta.record}}`);
  ...
  for (const prop in meta) {
    const metaTitle = headersText[prop];
  ...
  const headerRow = worksheet.addRow(
    ['date', 'time', 'user', 'type', 'displayName', 'new', 'old'].map(
      (t) => headersText[t]
    )
  );
