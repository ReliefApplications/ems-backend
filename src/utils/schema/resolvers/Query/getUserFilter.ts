 ...
 */
const buildUserMongoFilter = (filter: any, fields: any[], context: any): any => {
  if (filter.filters) {
    const filters = filter.filters.map((x: any) => buildUserMongoFilter(x, fields, context)).filter(x => x);
    if (filters.length > 0) {
      switch (filter.logic) {
        case 'and': {
          return { $and: filters };
