    ...
    for (const field of fieldsToExtend) {
      ...
      if (glRelatedField) {
        ...
        if (field.type === GraphQLID) {
          ...
          o += `extend type ${x} { ${glField}(filter: JSON, sortField: String, sortOrder: String, sortFirst: Int): [${glRelatedType}] }`;
        }
        o += `extend type ${metaName} { ${glField}: ${glRelatedMetaType} }`;
        if (!extendedFields.includes(key)) {
          o += `extend type ${glRelatedType} { ${glRelatedField}(filter: JSON, sortField: String, sortOrder: String, sortFirst: Int): [${x}] }
